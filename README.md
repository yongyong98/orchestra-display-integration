...
다음 원칙은 위 상태 시점을 기존 로봇 runtime에 적용할 때의 책임 경계와 검증
기준입니다.

- `RobotDisplay`는 프로그램 시작 시 한 번 생성하고 종료 시 `close()`합니다.
- `state()`는 실제 상태 전환 지점에서 한 번만 호출하며 매 프레임이나 제어 주기에는 호출하지 않습니다.
- `WAITING_FOR_RELEASE`는 기존 runtime에 안정적인 인계 신호가 있을 때만 사용합니다. 연동을 위해 새로운 힘·거리 임계값을 추가하지 않습니다.
- `SAFE_WAIT`는 복구 가능한 명시적 hold에만 사용하고, 실행을 종료하는 예외는 `ERROR`로 구분합니다.
- SDK 반환값, 태블릿 응답, Wi-Fi 상태는 로봇 모션의 실행·정지 조건으로 사용하지 않습니다. 기존 인식·계획·모션 로직과 안전 조건도 변경하지 않습니다.
- 구현 완료 후 각 상태의 연결 위치와 선행 조건을 `file:function` 기준으로 기록하고, simulator와 Y700에서 정상·오류 흐름을 검증합니다.
